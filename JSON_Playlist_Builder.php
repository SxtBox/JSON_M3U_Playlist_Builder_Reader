<?php
// JSON BUILDER BY FOLDER
$json_structure = '{
	"Albdroid_Streaming_API":{
	"Stream_Wrapper_Data":[';
$Playlist_Data = "Playlist_Data"; // SET PLAYLIST FOLDER TO SCAN
    ...
    foreach (scandir($folder) as $playlist){
        if(is_dir($playlist)){
            if($playlist!='..' && $playlist!='.'){
                $json_structure.='{"title":"'.$playlist.'","folder":';
...
function JSON_Playlist_Builder($playlist,$folder){
    ...
    if($playlist){
        ...
        $disallow_extensions = [".php",".jpg",".py",".js"]; // disable extensions
...
$json_structure = chop($json_structure,',')."\n".']'."\n".'}'."\n".'}'; // 
echo($json_structure);
file_put_contents('JSON_Playlist_Data.json', $json_structure);
?>